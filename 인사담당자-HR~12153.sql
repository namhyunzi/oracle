...
집합 연산자
    ...
    보통 서로 다른 테이블에 유사한 형태의 결과를 반환하는 것을 하나의 결과로 합칠 때 사용한다.
...
-- WHERE 값 연산자 (서브쿼리) : 제시된 값과 비교가능한 의미있는 값을 서브쿼리가 제공해야 한다.
-- WHERE EXISTS     (서브쿼리) : 서브쿼리는 의미있는 값을 제공하지 않아도 된다.
--                             의미없는 상수값을 반환해도 된다.

-- MINUS
-- 직종이 한번도 변경된 적이 없는 직원정보를 조회하기
SELECT EMPLOYEE_ID
FROM EMPLOYEES
MINUS
SELECT EMPLOYEE_ID
